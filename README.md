<h1 align="center">👋 ¡Hola! Soy Axel Aragón</h1>
<p align="center">Desarrollador en formación · SQL · JavaScript · C# · Python</p>

<!-- Pequeña franja de color con tecnologías -->
<p align="center">
  <img alt="SQL" src="https://img.shields.io/badge/SQL-336791.svg?logo=postgresql&logoColor=white">
  <img alt="JavaScript" src="https://img.shields.io/badge/JavaScript-F7DF1E.svg?logo=javascript&logoColor=black">
  <img alt="C#" src="https://img.shields.io/badge/C%23-239120.svg?logo=c-sharp&logoColor=white">
  <img alt="Python" src="https://img.shields.io/badge/Python-3776AB.svg?logo=python&logoColor=white">
  <img alt="NestJS" src="https://img.shields.io/badge/NestJS-E0234E.svg?logo=nestjs&logoColor=white">
  <img alt="HTML5" src="https://img.shields.io/badge/HTML5-E34F26.svg?logo=html5&logoColor=white">
</p>

<p align="center">
  <a href="mailto:axelaragonemes@gmail.com">Correo</a> ·
  <a href="https://www.linkedin.com/in/tu-user">LinkedIn</a> ·
  <a href="https://github.com/Axelaragon03">GitHub</a> ·
</p>

---

### Sobre mí
Soy una persona apasionada por la tecnología y el aprendizaje constante.  
Me gusta entender cómo funcionan las cosas desde dentro y mejorar un poco cada día.  
Actualmente estoy dando mis primeros pasos en el desarrollo **backend y frontend**,  
explorando diferentes lenguajes y frameworks.

---

### 🚀 En qué estoy enfocado
- Fortalecer mis bases en **JavaScript** y **C#**  
- Aprender buenas prácticas en **NestJS** y **Strapi**  
- Mejorar mi lógica y comprensión de **bases de datos SQL**  
- Crear pequeños proyectos personales que me ayuden a practicar lo aprendido  
- Contribuir en proyectos open source cuando adquiera más experiencia  

---

### 🧰 Tecnologías y herramientas
`SQL` · `JavaScript` · `C#` · `Python` · `NestJS` · `Strapi` · `HTML` · `CSS` · `Git`  

---

### 📚 Actualmente aprendiendo
- Fundamentos de **Frontend moderno** (HTML, CSS, JS)  
- **APIs con NestJS y Strapi**  
- **Lógica y estructuras de datos**  
- **Control de versiones con Git y GitHub**

---

### 💬 Intereses
Me gusta aprender de los demás, experimentar con nuevas tecnologías y trabajar en equipo.  
Creo firmemente en que cada proyecto, sin importar su tamaño, es una oportunidad para crecer.

---

### 📫 Contacto
<p align="center">
  <a href="mailto:axelaragonemes@gmail.com">
    <img alt="Email" src="https://img.shields.io/badge/Email-D14836.svg?logo=gmail&logoColor=white">
  </a>
  <a href="https://www.linkedin.com/in/tu-user">
    <img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-0077B5.svg?logo=linkedin&logoColor=white">
  </a>
  <a href="https://github.com/Axelaragon03">
    <img alt="GitHub" src="https://img.shields.io/badge/GitHub-181717.svg?logo=github&logoColor=white">
  </a>
</p>



